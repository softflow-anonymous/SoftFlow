...
def generate(n_samples, csp, color, save_path, title):
    ...
    with torch.no_grad():
        for ii in torch.split(inds, int(100**2)):
            std = torch.ones(z[ii].shape[0], 1).to(z)
            std_in = std / args.std_max * args.std_weight
            sample.append(softflow(z[ii], std_in * csp, reverse=True))
    sample = torch.cat(sample, 0).cpu().numpy()


    save_fig(sample, color, 'D', 0.1, save_path, title)

if __name__ == '__main__':
    n_samples = 2000
    COLOR = 1, 1, 1, 0.64
    save_path = 'generate2/' + args.data
    ...
    csp_list = [0.0, 0.02, 0.04, 0.06, 0.08, 0.1]
    for csp in csp_list:
        generate(n_samples, csp, COLOR, save_path, 'sample_softflow_csp-'+str(csp))
